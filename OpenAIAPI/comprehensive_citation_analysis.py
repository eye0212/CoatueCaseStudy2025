# Comprehensive Citation Analysis - All Contexts
# Ensures we capture citation patterns across all major use cases
...
MODEL = "gpt-4o-mini"
NUM_QUERIES = 60  # Balanced for comprehensive coverage
TEMPERATURE = 0.7
DELAY_BETWEEN_CALLS = 1.0
RAW_LOG = "comprehensive_citation_log.csv"
SUMMARY_FILE = "comprehensive_citation_summary.csv"
CHART_FILE = "comprehensive_citation_trends.png"
...
# Comprehensive prompt templates covering all major contexts
COMPREHENSIVE_PROMPTS = {
    # Educational/Factual Context (should trigger Wikipedia)
    "educational": [
    ...
    # Social/Community Context (should trigger Reddit, forums)
    "social": [
    ...
    # Problem-Solving Context (should trigger Stack Overflow, GitHub)
    "problem_solving": [
    ...
    # News/Current Events Context (should trigger news sites)
    "news": [
    ...
    # Shopping/Reviews Context (should trigger e-commerce)
    "shopping": [
    ...
    # Learning/Educational Context (should trigger educational platforms)
    "learning": [
    ...
    # Health/Medical Context (should trigger health sites)
    "health": [
    ...
    # Finance/Business Context (should trigger finance sites)
    "finance": [
    ...
    # Entertainment Context (should trigger entertainment platforms)
    "entertainment": [
...
COMPREHENSIVE_TOPICS = {
    "educational": [
        ...
        "machine learning", "solar system", "DNA", "gravity"
    ],
    
    "social": [
        "relationships", "mental health", "parenting", "dating", "friendship",
        "work-life balance", "social anxiety", "community building"
    ],
...
def main():
    print(f"\nRunning COMPREHENSIVE citation analysis for {NUM_QUERIES} queries...")
    print("This analysis covers ALL major contexts to ensure complete accuracy.\n")
    ...
    queries_per_context = NUM_QUERIES // len(COMPREHENSIVE_PROMPTS)
    remaining_queries = NUM_QUERIES % len(COMPREHENSIVE_PROMPTS)
    
    query_count = 0
    
    for context, prompts in COMPREHENSIVE_PROMPTS.items():
        topics = COMPREHENSIVE_TOPICS[context]
    ...
    context_analysis = []
    for context in COMPREHENSIVE_PROMPTS.keys():
    ...
    context_df.to_csv("context_analysis.csv", index=False, encoding="utf-8-sig")
    ...
    plt.title("Comprehensive Citation Analysis - All Contexts")
    plt.ylabel("Share (%)")
    plt.xticks(rotation=45, ha="right")
    
    # Context breakdown
    plt.subplot(2, 1, 2)
    context_counts = [context_counter[ctx] for ctx in COMPREHENSIVE_PROMPTS.keys()]
    plt.bar(COMPREHENSIVE_PROMPTS.keys(), context_counts, color="lightcoral")
    ...
    print(f"\nContext-specific top domains:")
    for _, row in context_df.iterrows():
        print(f"  {row['context']}: {row['top_domain']} ({row['top_domain_%']:.1f}%)")
    
    print(f"\nFiles saved:")
    print(f"  Detailed log: {RAW_LOG}")
    print(f"  Summary: {SUMMARY_FILE}")
    print(f"  Context analysis: context_analysis.csv")
